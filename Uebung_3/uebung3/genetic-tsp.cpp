...
int tourLength(const vector<int>& T) {
	...
	int sum_citydistance = 0;
	
	for (int i = 0; i < T.size(); i++)
	{
		sum_citydistance += (i < T.size() - 1) ? cityDistance(T[i], T[i + 1]) : cityDistance(T.front(), T.back());
	} 
	
	return sum_citydistance;
}
...
void crossover(const vector<int>& parent1, const vector<int>& parent2, vector<int>& child) {
	...
	copy(parent1.begin() + borderA, parent1.begin() + borderB, child.begin() + borderA);
	
	for (const int& current_city : parent2)
	{
		if (!findCity(child, current_city))
		{
			insertCity(child, current_city);
		}
	}
...
int main(int argc, char** argv) {
	...
	for(int e=0; e<5000; e++) {
		...
		maxDist = max(maxDist, lengths.second);
