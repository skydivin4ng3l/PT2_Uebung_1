...
void removeNonDirectFlights(std::map<int, AirportInfo>& airportInfo)
{
	...
	for (auto & airport : airportInfo){
			airport.second.m_routes.erase(std::remove_if(airport.second.m_routes.begin(),airport.second.m_routes.end(),
																		[](std::pair<int,int> routes) {return routes.second > 0;})
...
void calculateDistancePerRoute(std::map<int, AirportInfo>& airportInfo)
{
	...
	//std::back_inserter needed, because vector routeLength would be empty otherwise
	for(auto &airport : airportInfo){
		std::transform(airport.second.m_routes.begin(),airport.second.m_routes.end(),std::back_inserter(airport.second.m_routeLengths),
									 [&airport, &airportInfo](std::pair<int,int> routes)
									 { return calculateDistanceBetween(
										 	airport.second.pos[0], airport.second.pos[1],
											airportInfo[routes.first].pos[0], airportInfo[routes.first].pos[1]);
									 });
	}
...
void calculateAverageRouteDistances(std::map<int, AirportInfo>& airportInfo)
{
	...
	for(auto &airport: airportInfo){
			float distance = std::accumulate(airport.second.m_routeLengths.begin(),airport.second.m_routeLengths.end(),0.0f); //[](float dis, float routeLen){return dis + routeLen;} could be used in accumulate... but its not necessary

			airport.second.m_averageRouteLength = distance/(float)(airport.second.m_routeLengths.size());
		}
